...
fn main() -> Result<(), String> {
    ...
    // Get
    let my_timetable_rec = data_source::timetable::get_async(
        data_source::timetable::Query::ThisWeek,
        my_course
    );
    let my_timetable_next_rec = data_source::timetable::get_async(
        data_source::timetable::Query::NextWeek,
        my_course
    );
    let canteen_plan_rec = data_source::canteen_plan::get_async(
        data_source::canteen_plan::Query::NextWeek,
    );
    let canteen_plan_next_rec = data_source::canteen_plan::get_async(
        data_source::canteen_plan::Query::NextWeek,
    );
    ...
    loop {
        match my_timetable_rec.try_recv() {
            Ok(Ok(content)) =>
                my_timetable.extend(content),
            Ok(Err(e)) =>
                errors += &format!("Error getting timetable: \t{}", e),
            Err(_) =>
                (),
        }
        ...
        match my_timetable_next_rec.try_recv() {
            Ok(Ok(content)) =>
                my_timetable.extend(content),
            Ok(Err(e)) =>
                errors += &format!("Error getting timetable of next week: \t{}", e),
            Err(_) =>
                (),
        }
        ...
        match canteen_plan_rec.try_recv() {
            Ok(Ok(content)) =>
                canteen_plan.extend(content),
            Ok(Err(e)) =>
                errors += &format!("Error getting canteen plan: \t{}", e),
            Err(_) =>
                (),
        }
        ...
        match canteen_plan_next_rec.try_recv() {
            Ok(Ok(content)) =>
                canteen_plan.extend(content),
            Ok(Err(e)) =>
                errors += &format!("Error getting canteen plan of next week: \t{}", e),
            Err(_) =>
                (),
        }
...
fn render(today : &Date<Local>, canteen : &HashMap<Date<Local>, Vec<String>>, timetable :  &HashMap<Date<Local>, Vec<String>>) {
    ...
    Hochschul-App {}\n\n\
    {:10} {:02}.{:02}.{}{}
    ", VERSION, german_weekday(today.weekday()), today.day(), today.month(), today.year(), loading);

    let heading = VBox(NONE_BOX, Color::None,
                       Backgound(BASE2,
                           Margin((2,1),
...
mod test {
    ...
    pub(crate) fn test_render(_state: isize) {
        ...
        let mut root = GridV::new()
            .add(
                GridH::new().add(
                    VBox(SIMPLE_BOX, Color::BrightYellow, VText::simple("1"))
                ).add(
                    VBox(SIMPLE_BOX, Color::BrightYellow, VText::simple("2"))
                ).add(
                    VBox(SIMPLE_BOX, Color::BrightYellow, VText::simple("3"))
                )
            ).add(
                GridH::new().add(
                    VBox(SIMPLE_BOX, Color::BrightYellow, VText::simple("4"))
                ).add(
                    VBox(SIMPLE_BOX, Color::BrightYellow, VText::simple("5/6"))
                )
            ) .add(
                GridH::new().add(
                    VBox(SIMPLE_BOX, Color::BrightYellow, VText::simple("Full"))
                )
            );
    ...
    pub(crate) fn test_render2(state: &isize) {
        ...
        let mut root = GridH::new()
            .add(
                VBox(DOUBLE_BORDER_BOX, Color::BrightYellow,
                     Spacer::new(
                         VText::simple("Hello World")
                     )
                )
            ).add(
            VBox(SIMPLE_BOX, Color::BrightYellow,
                 Spacer::new(
                     VText::simple("Hello World. And also: 'Hello Humanity'. And Stuff... This is Filler Text")
                 )
            )
        ).add(
            VBox(BORDER_BOX, Color::BrightYellow,
                 Margin((4, 2), Backgound(Color::BrightBlue,
                                          Spacer::new(
                                              VText::simple("\t1\t2\t3\t4\t5\t6\t7\t8\t9\t\n\ntab stops are working!")
                                          )
                 ))
            )
        ).add(
            VText::simple(&format!("Tic: {:.2}", *state as f32 / 1000f32))
        );
